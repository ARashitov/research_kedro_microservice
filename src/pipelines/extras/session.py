...
def _activate_session(session: "KedroSession", force: bool = False) -> None:
    ...
    if _active_session and not force and session is not _active_session:
        raise RuntimeError("Cannot activate the session as another active session already exists.")
...
class KedroSession:
    ...
    def create(  # pylint: disable=too-many-arguments
        ...
        session_data["extra_params"] = {"trace_id": trace_id}
    ...
    def _init_store(self) -> BaseSessionStore:
        ...
        try:
            ...
            raise ValueError(
                f"\n{err}.\nStore config must only contain arguments valid " f"for the constructor of `{classpath}`.",
            ) from err
    ...
    def run(
        ...
        extend_catalog: Dict[str, AbstractDataSet] = None,
    ) -> Dict[str, Any]:
        ...
        # NOTE: here is extra functionality extending catalog content
        if extend_catalog is not None:
            catalog.add_all(extend_catalog, replace=True)
